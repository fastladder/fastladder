
// manage

updater('manage_item', function () {
    if (!Manage.Item.loaded) {
        Manage.Item.load();
        return;
    }
    const data = Manage.Item.get_items();
    const tmpl = Template.get('man_items').compile();
    const now = new Date() / 1000;
    const fmt = function (item) {
        const sid = item.subscribe_id;
        item = subs_item(sid);
        const classname = TRSelector.cart.has(sid) ? 'selected' : '';
        return tmpl(item, {
            update: (now - item.modified_on).toRelativeDate(),
            notify_text: item.ignore_notify ? '無効' : '有効',
            classname,
        });
    };
    const param = {};
    param[`sortmode_${MI.sort_mode}`] = 'selected';
    this.innerHTML = [
        Template.get('man_item_header').fill({
            has_prev: Manage.Item.has_prev() ? '' : 'disable',
            has_next: Manage.Item.has_next() ? '' : 'disable',
        }, param),
        data.list.slice(MI.offset, MI.offset + MI.perpage).map(fmt).join(''),
        '</table></div>',
    ].join('');
    addEvent(_$('manage_table'), 'selectstart', Event.stop);
    addEvent(_$('manage_table'), 'mousedown', Event.stop);
    update('manage_select', 'move_to', 'manage_offset');
});

updater('manage_select', () => {
    const size = TRSelector.cart.keys.length;
    if (size) {
        if (app.state.manage_disabled) {
            Form.enable_all('manage_control');
            removeClass('manage_control', 'grayout');
            app.state.manage_disabled = false;
        }
        Manage.message(size + I18n.t(' items selected'));
    } else {
        Manage.message(I18n.t('Select item(s) you want to edit'));
        addClass('manage_control', 'grayout');
        Form.disable_all('manage_control');
        app.state.manage_disabled = true;
    }
});

const Selector = Class.create().extend({
    initialize() {
        const self = this;
        Object.extend(this, {
            mousedown(event) { self.onmousedown.call(self, this, event); },
            mouseover(event) { self.onmouseover.call(self, this, event); },
            mouseout(event) { self.onmouseout.call(self, this, event); },
        });
        self.setup && self.setup.apply(this, arguments);
    },
    onmousedown(el, event) {
        app.state.mdown = true;
        if (hasClass(el, 'selected')) {
            removeClass(el, 'selected');
            app.state.turn = false;
        } else {
            addClass(el, 'selected');
            app.state.turn = true;
        }
    },
    onmouseover(el, event) {
        if (app.state.mdown) {
            (app.state.turn) ? addClass(el, 'selected') : removeClass(el, 'selected');
        } else {
            addClass(el, 'focus');
        }
    },
    onmouseout(el, event) {
        removeClass(el, 'focus');
    },
});
const Cart = Class.create();
Cart.extend({
    initialize() {
        this.hash = {};
        this.keys = [];
        this.values = [];
    },
    clear() {
        return this.initialize();
    },
    has(key) {
        return this.hash.hasOwnProperty(`item_${key}`);
    },
    add(key, value) {
        if (!this.has(key)) {
            this.hash[`item_${key}`] = value;
            this.keys.push(key);
            this.values.push(key);
        }
    },
    remove(key) {
        if (!this.has(key)) return false;
        delete this.hash[`item_${key}`];
        const idx = this.keys.indexOf(key);
        this.keys.delete_at(idx);
        this.values.delete_at(idx);
        return this;
    },
    get(key) {
        return this.hash(`item_${key}`);
    },
});
const SelectorWithCart = Class.create().extend({
    initialize() {
        this.cart = new Cart();
    },
    clear() {
        this.cart.clear();
    },
    _updateCart(element) {
        let change;
        const sid = element.getAttribute('subscribe_id') - 0;
        const chk = hasClass(element, 'selected');
        if (!chk) {
            change = this.cart.remove(sid);
        } else if (!this.cart.has(sid)) {
            this.cart.add(sid);
            change = true;
        }
        change && update('manage_select');
    },
    onmousedown() {
        this._updateCart.apply(this, arguments);
    },
    onmouseover() {
        this._updateCart.apply(this, arguments);
    },
    get_selected() {
        return this.cart.keys;
    },
});
const ItemSelector = Class.merge(Selector, SelectorWithCart);
var TRSelector = new ItemSelector();

var Manage = {};
Manage.message = function (str) {
    _$('manage_select').innerHTML = str;
};

Manage.Item = {
    get_items() {
        return this.filtered || this.data;
    },
    offset: 0,
    perpage: 20,
    filter: null,
    filtered: null,
    set_filter(f) { this.filter = f; },
    reload_filter() {
        if (this.filter) {
            this.filtered = this.data.filter(this.filter);
        }
        this.update();
    },
    loaded: false,
    load() {
        const self = this;
        this.data = new Subscribe.Model();
        new LDR.API('/api/subs?unread=0').post(
            {},
            (list) => {
                self.loaded = true;
                self.data.load(list);
                if (self.sort_mode) {
                    self.do_sort();
                }
                if (self.filter) {
                    self.reload_filter();
                } else {
                    self.update();
                }
            },
        );
    },
    search() {
        const q = this.value;
        if (q == '') {
            MI.filtered = null;
            MI.update();
            return;
        }
        MI.filtered = MI.data.filter(item => contain(item.title, q));
        MI.update();
    },
    do_sort() {
        if (!this.sort_mode) return;
        const data = this.get_items();
        data.list.sort_by(this.sort_mode);
    },
    sort_mode: null,
    sort(sort_mode) {
        const data = this.get_items();
        if (sort_mode == this.sort_mode) {
            data.list.reverse();
            this.update();
            return;
        }
        this.sort_mode = sort_mode;
        this.do_sort();
        this.update();
    },
    clear_select() {
        TRSelector.cart.clear();
        this.update();
    },
    select_all() {
        const data = this.get_items();
        const cart = TRSelector.cart;
        foreach(data.list, (item) => {
            const sid = item.subscribe_id;
            cart.add(sid);
        });
        this.update();
    },
    page_select() {
        const data = this.get_items();
        const cart = TRSelector.cart;
        data.list.slice(MI.offset, MI.offset + MI.perpage).forEach((item) => {
            const sid = item.subscribe_id;
            if (cart.has(sid)) {
                cart.remove(sid);
            } else {
                cart.add(sid);
            }
        });
        this.update();
    },
    reverse_select() {
        const data = this.get_items();
        const cart = TRSelector.cart;
        foreach(data.list, (item) => {
            const sid = item.subscribe_id;
            if (cart.has(sid)) {
                cart.remove(sid);
            } else {
                cart.add(sid);
            }
        });
        this.update();
    },
    data: null,
    has_prev() {
        return this.offset > 0;
    },
    has_next() {
        const data = this.get_items();
        return this.offset + this.perpage < data.list.length;
    },
    prev() {
        if (this.has_prev()) {
            this.offset = Math.max(0, this.offset - this.perpage);
            update('manage_item', 'mi_paging');
        }
    },
    next() {
        if (this.has_next()) {
            this.offset += this.perpage;
            update('manage_item', 'mi_paging');
        }
    },
    update() {
        update('manage_item', 'mi_paging');
    },
    do_move() {
        const sel = _$('move_to');
        const to = Form.getValue(sel);
        const ids = TRSelector.get_selected();
        move_to(ids.join(','), to);
        foreach(ids, (sid) => {
            const item = subs_item(sid);
            if (item) item.folder = to;
        });
        update('manage_item');
    },

    toggle_notify() {
        const ids = TRSelector.get_selected();
        if (!ids.length) return;
        let turn = 0;
        const sids = [];
        foreach(ids, (sid, n) => {
            const item = subs_item(sid);
            if (!item) return;
            if (n == 0) { turn = item.ignore_notify ? 0 : 1; }
            item.ignore_notify = turn;
            sids.push(sid);
        });
        const api = new LDR.API('/api/feed/set_notify');
        api.post({
            subscribe_id: sids.join(','),
            ignore: turn,
        }, () => {
            message('通知設定を変更しました');
        });
        update('manage_item');
    },

    unsubscribe() {
        const ids = TRSelector.get_selected();
        let l = ids.length;
        const tmpl_confirm = I18n.t('manage_unsubscribe_confirm_tmpl');
        const tmpl_progress = I18n.t('manage_unsubscribe_progress_tmpl');
        const c = confirm(tmpl_confirm.fill({ count: l }));
        if (!c) return;
        TRSelector.clear();
        foreach(ids, (sid, n) => {
            const api = new LDR.API('/api/feed/unsubscribe');
            api.post({ subscribe_id: sid }, () => {
                l--;
                message(tmpl_progress.fill({ remain: l }));
                if (l == 0) {
                    message(I18n.t('Feeds deleted'));
                    MI.load();
                }
            });
        });
    },

    touch() {
        const ids = TRSelector.get_selected();
        foreach(ids, (sid) => {
            const item = subs_item(sid);
            if (!item) return;
            if (item.unread_count > 0) {
                item.unread_count = 0;
                const api = new LDR.API('/api/touch_all');
                api.post({ subscribe_id: sid });
            }
        });
        message('Marked as read');
        update('manage_item');
    },

};

/*
 再描画
*/
Manage.rewrite = {

};

updater('manage_offset', function () {
    const tmpl = Template.get('man_offset').compile();
    const folder_id = MF.folder_id;
    if (folder_id == 0) {
        var folder_name = I18n.t('Uncategolized');
    } else if (!folder_id) {
        var folder_name = I18n.t('All');
    } else {
        var folder_name = folder.id2name[folder_id];
    }
    const size = MI.get_items().list.length;
    this.innerHTML = tmpl({
        selecter_name: folder_name,
        size,
        start: MI.offset + 1,
        end: Math.min(MI.offset + MI.perpage, size),
    });
});


updater('mi_paging', () => {
    const prev = _$('mi_prev');
    const next = _$('mi_next');
    MI.has_next() ? removeClass(next, 'disable') : addClass(next, 'disable');
    MI.has_prev() ? removeClass(prev, 'disable') : addClass(prev, 'disable');
});


updater('move_to', function () {
    this.options.length = 0;
    this.options[0] = new Option(I18n.t('Uncategolized'), '');
    const op = this.options;
    folder.names.map((v, i) => {
        op[i + 1] = new Option(v, v);
    });
});


// folderの管理

updater('manage_folder', () => {
    if (MF.change_flag || !folder) {
        MF.change_flag = false;
        get_folders(updater('manage_folder'));
        return;
    }
    const tmpl = Template.get('man_folder');
    const fmt = function (v) {
        const id = folder.name2id[v];
        return tmpl.fill({
            folder: v,
            id,
            selected: MF.folder_id == id ? 'selected' : '',
        });
    };
    _$('manage_folder').innerHTML = [
        `<li class="button ${MF.folder_id == 0 ? 'selected' : ''}" onclick="MF.select(0)">`, I18n.t('Uncategolized'), '</li>',
        folder.names.map(fmt).join(' '),
    ].join('');
    update('update_folder');
    update('move_to');
});
updater('update_folder', function () {
    if (!MF.folder_id) {
        this.innerHTML = '';
        return;
    }
    const tmpl = Template.get('manage_form').compile();
    this.innerHTML = tmpl({
        folder_id: MF.folder_id,
        folder_name: folder.id2name[MF.folder_id],
    });

    ajaxize('rename_form', (res, req) => {
        message(`${folder.id2name[req.folder_id]}-&gt;${req.name}`);
        folder = null;
        MF.change_flag = true;
        update('manage_folder');
        // フィルタを変更
        MI.set_filter(item => item.folder == req.name);
        MI.load();
    });

    ajaxize('delete_form', {
        before(param) {
            const tmpl = I18n.t('manage_folder_delete_confirm');
            const c = confirm(tmpl.fill({ folder: folder.id2name[param.folder_id] }));
            return !!c;
        },
        after(res, req) {
            const fn = folder.id2name[req.folder_id];
            message(fn + I18n.t(' deleted'));
            MF.folder_id = null;
            folder = null;
            // フォルダ一覧を再読み込みして表示のみ更新
            const rewrite = function () {
                update('manage_folder');
                // folderを削除
                MI.data.list.filter(item => item.folder == fn).forEach((item) => {
                    const sid = item.subscribe_id;
                    subs_item(sid).folder = '';
                });
                MI.update();
            };
            get_folders(rewrite);
        },
    });
});

Manage.Folder = {
    folder_id: null,
    change_flag: false,

    select(id) {
        if (this.folder_id == id) {
            this.folder_id = null;
        } else {
            this.folder_id = id;
        }
        // offsetを初期化
        MI.offset = 0;
        update('manage_folder');
        update('update_folder');
        // itemも更新
        if (MI.loaded) {
            if (this.folder_id == null) {
                MI.filtered = null;
            } else if (this.folder_id == 0) {
                MI.set_filter(item => item.folder == '');
                return MI.reload_filter();
            } else {
                const folder_name = folder.id2name[id];
                MI.set_filter(item => item.folder == folder_name);
                return MI.reload_filter();
            }
            MI.do_sort();
            MI.update();
        }
    },
    filter() { return true; },
    create_folder(callback) {
        const name = prompt(I18n.t('Folder Name'), '');
        if (!name) return;
        const api = new LDR.API('/api/folder/create');
        api.post({ name }, (res) => {
            message('folder created');
            folder = null;
            callback();
        });
    },

    rename_folder(callback) {
        const folder_id = MF.folder_id;
        if (!folder_id) return false;
        const new_name = _$('rename_to').value;
        if (!new_name) return false;
        const api = new LDR.API('/api/folder/update');
        api.post({ folder_id, name: new_name }, () => {
            message(`${folder.id2name[folder_id]}-&gt;${new_name}`);
            folder = null; callback();
        });
        MF.change_flag = true;
        return true;
    },

    delete_folder(callback) {
        const folder_id = MF.folder_id;
        if (!folder_id) return false;
        const tmpl = I18n.t('manage_folder_delete_confirm');
        const c = confirm(tmpl.fill({ folder: folder.id2name[folder_id] }));
        if (!c) return false;
        const api = new LDR.API('/api/folder/delete');
        api.post({ folder_id }, () => {
            message(folder.id2name[folder_id] + I18n.t(' deleted'));
            MF.folder_id = null;
            folder = null;
            callback();
        });
        MF.change_flag = true;
        return true;
    },
};

Manage.show_help = function () {
    if (this.disabled) return;
    _$('manage_help').innerHTML = `-&gt; &nbsp;&nbsp;${this.title}`;
};
Manage.hide_help = function () {
    _$('manage_help').innerHTML = '';
};


// deleteを実行
function delete_folder(folder_id) {
    const api = new LDR.API('/api/folder/delete');
    api.post({ folder_id }, () => {
        message(folder.id2name[folder_id] + I18n.t(' deleted'));
        folder = null; callback();
    });
    const models = SM.instances;
    const folder_name = folder.id2name[folder_id];
    models.invoke('delete_folder', folder_name);
}

// renameを実行
function rename_folder(folder_id, new_name) {
    const api = new LDR.API('/api/folder/update');
    api.post({ folder_id, name: new_name }, () => {
        message(`${folder.id2name[folder_id]}-&gt;${new_name}`);
        folder = null; callback();
    });
    const models = SM.instances;
    const folder_name = folder.id2name[folder_id];
    models.invoke('rename_folder', folder_name, new_name);
}


var MI = Manage.Item;
var MF = Manage.Folder;

MouseUp = new Trigger('mouseup');
MouseUp.add(True, () => { app.state.mdown = false; });
MouseUp.apply();


// 描画領域を入れ替える。
function change_buffer() {
    const current_buffer = _$('right_body');
    const background = _$('buffer');

    current_buffer.id = 'buffer';
    current_buffer.style.display = 'none';

    background.id = 'right_body';
    background.style.display = 'block';
}
addEvent(_$('mini_window'), 'dblclick', (e) => {
    Event.stop(e);
    DOM.hide('mini_window');
});
function preview(sid) {
    print_feed.target = 'mini_window_body';
    DOM.show('mini_window');
    centering('mini_window');
    const item = subs_item(sid);
    if (true) {
        const api = new LDR.API('/api/all');
        api.onload = function (json) {
            print_feed(json);
        };
        api.post({
            subscribe_id: sid,
            limit: 1,
        });
    }
}
function centering(element, x, y) {
    element = _$(element);
    x = x || 0;
    y = y || 0;
    const w = element.offsetWidth;
    const h = element.offsetHeight;
    const bw = document.body.offsetWidth;
    const bh = document.body.offsetHeight;
    const top = `${(bh / 2) - (h / 2) - y}px`;
    const left = `${(bw / 2) - (w / 2) - x}px`;
    setStyle(element, {
        top,
        left,
    });
}
updater('config_form', () => {
    const active = TabManager.config_form;
    (active == 'config_basic') ?
        Element.hide('config_submit') :
        Element.show('config_submit');
});

